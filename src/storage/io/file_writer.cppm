...
public:
    explicit FileWriter(FileSystem &fs, const std::string &path, SizeT buffer_size, u8 file_flags = FileFlags::WRITE_FLAG | FileFlags::CREATE_FLAG)
        : fs_(fs), path_(path), data_(std::make_unique<char_t[]>(buffer_size)), offset_(0), total_written_(0), buffer_size_(buffer_size) {
